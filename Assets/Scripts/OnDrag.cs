...
public class OnDrag : MonoBehaviour
{
    ...
    private float min_Y = -45f;

    private float max_Y = 48f;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (gameObject.tag == collision.gameObject.tag)
        {
            ...
            {
                GameManager.Instance.swordMans.Remove(collision.gameObject);
                GameManager.Instance.swordMans.Remove(gameObject);
                ...
                GameObject swordmanRespawn = (GameObject)Instantiate(Resources.Load("swordman"), transform.position, Quaternion.identity);
                swordmanRespawn.GetComponent<Player>().PowerUp(gameObject.GetComponent<Player>().power + collision.gameObject.GetComponent<Player>().power);
            }
            else if (collision.gameObject.tag == "archer")
            {
                GameManager.Instance.archerMans.Remove(collision.gameObject);
                GameManager.Instance.archerMans.Remove(gameObject);
                ...
                GameObject archermanRespawn = (GameObject)Instantiate(Resources.Load("archerman"), transform.position, Quaternion.identity);
                archermanRespawn.GetComponent<Player>().PowerUp(gameObject.GetComponent<Player>().power + collision.gameObject.GetComponent<Player>().power);
